...
You can use the `getnodeaddresses` RPC to fetch a number of onion peers known to your node; run `rcpu-cli help getnodeaddresses` for details.
...
To see verbose Tor information in the rcpud debug log, pass `-debug=tor`.
...
methods to be configured: cookie authentication or rcpud's `-torpassword`
configuration option.

#### Cookie authentication

For cookie authentication, the user running rcpud must have read access to
the `CookieAuthFile` specified in the Tor configuration. In some cases this is
preconfigured and the creation of an onion service is automatic. Don't forget to
use the `-debug=tor` rcpud configuration option to enable Tor debug logging.

If a permissions problem is seen in the debug log, e.g. `tor: Authentication
cookie /run/tor/control.authcookie could not be opened (check permissions)`, it
can be resolved by adding both the user running Tor and the user running
rcpud to the same Tor group and setting permissions appropriately.
...
Add these lines to your `/etc/tor/torrc` (or equivalent config file):
    ...
    HiddenServicePort 9965 127.0.0.1:9966

The directory can be different of course, but virtual port numbers should be equal to
your rcpud's P2P listen port (9965 by default), and target addresses and ports
...
In a typical situation, where you're only reachable via Tor, this should suffice:
    ...
    ./rcpud -proxy=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -listen

(obviously, replace the .onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

    ./rcpud ... -bind=127.0.0.1
...
and open port 9965 on your firewall (or use port mapping, i.e., `-upnp` or `-natpmp`).

If you only want to use Tor to reach .onion addresses, but not use it as a proxy
for normal IPv4/IPv6 communication, use:

    ./rcpud -onion=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -discover
