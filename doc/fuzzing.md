...
It is possible to specify `rcpud` arguments to the `fuzz` executable.
...
Core without having to write any custom fuzzing harness. The `rcpud` server
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/rcpud -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
