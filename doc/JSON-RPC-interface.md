...
The headless daemon `rcpud` has the JSON-RPC API enabled by default, the GUI
`rcpu-qt` has it disabled by default. This can be changed with the `-server`
...
This is the endpoint used by rcpu-cli when a `-rpcwallet=` parameter is passed in.
...
rcpu-cli createwallet mywallet false false "" false false true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
rcpu-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
rcpu-cli -named createwallet mywallet load_on_startup=true
```
...
- **Securing remote network access:** You may optionally allow other
  ...
  `rcpud -help` for more information about these settings and other
