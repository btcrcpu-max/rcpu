...
Below are some notes on how to build RCPU Core for Windows.

The options known to work for building RCPU Core on Windows are:
...
Note that for WSL the RCPU Core source path MUST be somewhere in the default mount file system, for
example /usr/src/RCPU, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\RCPU`, for example:

    make install DESTDIR=/mnt/c/workspace/RCPU
