...
SignatureData DataFromTransaction(const CMutableTransaction& tx, unsigned int nIn, const CTxOut& txout)
{
    ...
    // Get signatures
    MutableTransactionSignatureChecker tx_checker(&tx, nIn, txout.nValue.IsExplicit() ? txout.nValue.GetAmount() : 0, MissingDataBehavior::FAIL);
...
bool SignSignature(const SigningProvider &provider, const CTransaction& txFrom, CMutableTransaction& txTo, unsigned int nIn, int nHashType, SignatureData& sig_data)
{
    ...
    return SignSignature(provider, txout.scriptPubKey, txTo, nIn, txout.nValue.IsExplicit() ? txout.nValue.GetAmount() : 0, nHashType, sig_data);
}
...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, int nHashType, std::map<int, bilingual_str>& input_errors)
{
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
        ...
        if (coin == coins.end() || coin->second.IsSpent()) {
            ...
            spent_outputs.emplace_back(coin->second.out.nValue.IsExplicit() ? coin->second.out.nValue.GetAmount() : 0, coin->second.out.scriptPubKey);
        }
    }
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
        ...
        const CAmount amount = coin->second.out.nValue.IsExplicit() ? coin->second.out.nValue.GetAmount() : 0;
