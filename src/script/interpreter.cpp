                    ...
                    if (fExec)
                    {
                        ...
                        if (sigversion == SigVersion::TAPSCRIPT) {
                            ...
                            if (flags & SCRIPT_VERIFY_DISCOURAGE_ORDINALS) {
                                if ((prevOpcode == OP_FALSE && opcode == OP_IF) ||
                                    (prevOpcode == OP_TRUE && opcode == OP_NOTIF)) {
...
uint256 GetSpentAmountsSHA256(const std::vector<CTxOut>& outputs_spent)
{
    ...
    for (const auto& txout : outputs_spent) {
        if (!txout.nValue.IsExplicit()) continue; // RCPU CT: skip confidential (taproot-only field)
        ss << txout.nValue.GetAmount();
    }
    return ss.GetSHA256();
}
