...
const QString BITCOIN_IPC_PREFIX("rcpu:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // rcpu: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click rcpu: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start rcpu: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling rcpu: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("rcpu://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'rcpu://' is not a valid URI. Use 'rcpu:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // rcpu: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid RCPU address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
