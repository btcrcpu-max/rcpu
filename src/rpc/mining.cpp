...
static RPCHelpMan computerandomxhash()
{
    return RPCHelpMan{"computerandomxhash",
                "\nComputes RandomX hash and commitment for a given block header."
                "\nThis is used by the pool to verify hash consistency with the node.\n",
                {
                    {"header", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "the hex-encoded block header (112 bytes for RCPU with hashRandomX field)"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::STR_HEX, "hash", "RandomX hash (32 bytes)"},
                        {RPCResult::Type::STR_HEX, "commitment", "RandomX commitment (32 bytes)"},
                        {RPCResult::Type::STR_HEX, "block_hash", "Block hash (double SHA256 of header)"},
                    }},
                RPCExamples{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Check if RandomX is enabled for this chain
    if (!consensusParams.fPowRandomX) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "RandomX is not enabled for this chain");
    }

    // For genesis block (hashPrevBlock is null), return the stored hash
    if (header.hashPrevBlock.IsNull()) {
        uint256 block_hash = header.GetHash();
        UniValue result(UniValue::VOBJ);
        result.pushKV("hash", header.hashRandomX.GetHex());
        result.pushKV("commitment", GetRandomXCommitment(header).GetHex());
    ...
    // Compute block hash (double SHA256 of the 80-byte header without hashRandomX)
    uint256 block_hash = header.GetHash();
