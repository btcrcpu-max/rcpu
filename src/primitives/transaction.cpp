...
{
    return strprintf("CTxOut(nValue=%s, scriptPubKey=%s)", (nValue.IsExplicit() ? strprintf("%d.%08d", nValue.GetAmount() / COIN, nValue.GetAmount() % COIN) : std::string("CONFIDENTIAL")), HexStr(scriptPubKey).substr(0, 30));
}
...
CAmount CTransaction::GetValueOut() const
{
    ...
    for (const auto& tx_out : vout) {
        if (!tx_out.nValue.IsExplicit()) continue; // RCPU CT: skip confidential outputs
        // RCPU CT: fee outputs are not part of the transferable value
        if (tx_out.IsFee()) continue;
        if (!MoneyRange(tx_out.nValue.GetAmount()) || !MoneyRange(nValueOut + tx_out.nValue.GetAmount()))
...
CAmount CTransaction::GetFeeOut() const
{
    CAmount nFee = 0;
    for (const auto& tx_out : vout) {
        if (tx_out.IsFee()) {
            nFee += tx_out.nValue.GetAmount();
        }
    }
    return nFee;
}
