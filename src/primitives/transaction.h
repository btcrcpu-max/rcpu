...
public:
    ...
    CConfidentialValue nValue; //!< Confidential value commitment (CT mode only)
...
public:
    ...
    std::vector<unsigned char> vchRangeproof; //!< Range proof (CT mode only)
    ...
    inline void Serialize(Stream& s) const {
        if (g_ct_serialization) {
            ...
            s << (nValue.IsExplicit() ? nValue.GetAmount() : CAmount(0));
        }
        s << scriptPubKey;
    }
    template <typename Stream>
    ...
    bool IsNull() const
    {
        return (nValue.IsExplicit() && nValue.GetAmount() == -1);
    }
...
public:
    ...
    // RCPU CT: sum of explicit fee outputs (0 for non-CT transactions).
    CAmount GetFeeOut() const;
