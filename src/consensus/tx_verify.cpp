...
bool Consensus::CheckTxInputs(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee, int nCTActivationHeight)
{
    ...
    // Collect previous outputs (needed for both CT balance and the maturity check)
    ...
    if (g_con_elementsmode && tx.nVersion == CT_VERSION) {
        ...
        if (!VerifyAmounts(prevouts, tx, txfee)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-txns-ct-balance",
                "confidential value balance or range proof failed");
        }
        return true;
    }
    ...
    for (const CTxOut& prevout : prevouts) {
        ...
        nValueIn += prevout.nValue.GetAmount();
        if (!MoneyRange(prevout.nValue.GetAmount()) || !MoneyRange(nValueIn)) {
