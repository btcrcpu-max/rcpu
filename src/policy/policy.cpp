...
bool IsStandardTx(const CTransaction& tx, const std::optional<unsigned>& max_datacarrier_bytes, bool permit_bare_multisig, const CFeeRate& dust_relay_fee, std::string& reason)
{
    ...
    // RCPU: v3 transactions with all explicit outputs are non-standard
    // (use v2 instead - it's smaller and has the same semantics).
    // This is a policy rule only; consensus still accepts all-explicit v3 txs.
    ...
    for (const CTxOut& txout : tx.vout) {
        ...
        if (whichType == TxoutType::NULL_DATA)
            ...
            return false;
        } else if (txout.nValue.IsExplicit() && IsDust(txout, dust_relay_fee)) {
            reason = "dust";
            return false;
        }
    }
