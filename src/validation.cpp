...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // This should never trigger on RCPU networks (CT is enabled by default), but provides
    // a safe rejection rather than a crash if there is a misconfiguration.
...
bool CScriptCheck::operator()() {
    ...
    return VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, m_tx_out.nValue.IsExplicit() ? m_tx_out.nValue.GetAmount() : 0, cacheStore, *txdata), &error);
}
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // The genesis block has no ancestor to walk, and test fixtures connect it
    // through this same path, so guard instead of asserting (issue #6). Skipping
    // the BIP34 refinement for height 0 is safe: BIP30 needs two blocks with the
    // same coinbase, which cannot exist before any block has been mined.
    if (pindex->pprev) {
    ...
    const SnapshotMetadata& metadata)
{
    ...
    while (coins_left > 0) {
        ...
        if (coin.out.nValue.IsExplicit() && !MoneyRange(coin.out.nValue.GetAmount())) {
