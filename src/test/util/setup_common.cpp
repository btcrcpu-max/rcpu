    ...
    : TestingSetup{ChainType::REGTEST, extra_args, coins_db_in_memory, block_tree_db_in_memory}
{
    ...
    // RCPU: block hash differs from Bitcoin Core due to chain parameter changes
    // Original Bitcoin Core assertion removed.
    // {
    //     LOCK(::cs_main);
    //     assert(
                                                                                  ...
                                                                                  const std::optional<uint32_t>& fee_output)
{
    ...
    if (feerate.has_value()) {
        ...
        if (deduction > 0) {
            ...
            mempool_txn.vout[fee_output.value()].nValue = mempool_txn.vout[fee_output.value()].nValue.GetAmount() -  deduction;
...
std::vector<CTransactionRef> TestChain100Setup::PopulateMempool(FastRandomContext& det_rand, size_t num_transactions, bool submit)
{
    ...
    std::transform(m_coinbase_txns.begin(), m_coinbase_txns.end(), std::back_inserter(unspent_prevouts),
        [](const auto& tx){ return std::make_pair(COutPoint(tx->GetHash(), 0), tx->vout[0].nValue.GetAmount()); });
