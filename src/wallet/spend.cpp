                                            ...
                                            const CoinSelectionParams& coin_selection_params) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    for (const COutPoint& outpoint : coin_control.ListSelected()) {
        ...
        // RCPU CT: unblind wallet-owned confidential outputs so the preselect total reflects the real amount
        if (!txout.nValue.IsExplicit()) {
            const CAmount real = OutputGetCredit(wallet, txout, isminefilter(wallet.IsMine(txout)));
            if (real <= 0) {
                return util::Error{strprintf(_("Cannot determine real value of pre-selected input %s"), outpoint.ToString())};
            }
            output.SetValue(real);
        }
                           ...
                           const CoinFilterParams& params)
{
    ...
    for (const auto& entry : wallet.mapWallet)
    {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            // RCPU CT: get real value (unblind for our confidential outputs)
            CAmount output_real_value = output.nValue.IsExplicit() ? output.nValue.GetAmount() : OutputGetCredit(wallet, output, isminefilter(mine));
            // RCPU CT: skip outputs that cannot be unblinded (value unknown)
            ...
            // RCPU CT: set real value for our CT outputs
            if (!output.nValue.IsExplicit()) {
                coin.SetValue(output_real_value);
            }
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    const CAmount MAX_MIN_VIABLE_CHANGE = 1000000; // 0.01 RCPU
    coin_selection_params.min_viable_change = std::min(std::max(change_spend_fee + 1, dust), MAX_MIN_VIABLE_CHANGE);
    ...
    // RCPU CT: confidential outputs add a range proof (~675 bytes),
    // an ECDH nonce (33 bytes) and a value prefix byte.
    // Add this overhead to tx_noinputs_size so coin selection accounts for it.
    // Also account for the explicit fee output that is added after blinding.
    if (g_con_elementsmode) {
        coin_selection_params.tx_noinputs_size += static_cast<int>(txNew.vout.size()) * 700;
        // Fee output: explicit amount (9 bytes max) + empty scriptPubKey (1 byte) = ~10 bytes
    ...
    // RCPU CT: confidential outputs add a Bulletproofs range proof (~675 bytes),
    // an ECDH nonce (33 bytes) and a value prefix byte. Account for that overhead so
    // the fee meets the minimum relay fee after blinding (otherwise the transaction
    // is too large for its fee and is never broadcast).
    if (g_con_elementsmode) {
        nBytes += static_cast<int>(txNew.vout.size()) * 700;
    }
    ...
    // If there is a change output, adjust change to match the fee needed
    // (increase change if we overpay, decrease change if we underpay due to CT overhead)
    if (change_pos && fee_needed != current_fee) {
        auto& change = txNew.vout.at(*change_pos);
        CAmount fee_diff = current_fee - fee_needed;
        // fee_diff > 0: overpaying, increase change
        // fee_diff < 0: underpaying (e.g. CT overhead), decrease change
        if (change.nValue.IsExplicit()) { change.nValue = change.nValue.GetAmount() + fee_diff; }
        if (change.nValue.IsExplicit() && change.nValue.GetAmount() < 0) {
            return util::Error{_("Change output too small to cover CT fee overhead")};
        }
        current_fee = result.GetSelectedValue() - CalculateOutputValue(txNew);
        if (fee_needed != current_fee) {
            LogPrintf("WARNING: Change adjustment fee mismatch\n");
        }
    }
    ...
    if (coin_selection_params.m_subtract_fee_outputs) {
        ...
        for (const auto& recipient : vecSend)
        {
            ...
            if (recipient.fSubtractFeeFromAmount)
            {
                if (txout.nValue.IsExplicit()) { txout.nValue = txout.nValue.GetAmount() - to_reduce / outputs_to_subtract_fee_from; } // Subtract fee equally from each selected recipient

                if (fFirst) // first receiver pays the remainder not divisible by output count
                {
                    fFirst = false;
                    if (txout.nValue.IsExplicit()) { txout.nValue = txout.nValue.GetAmount() - to_reduce % outputs_to_subtract_fee_from; }
                }

                // Error if this output is reduced to be below dust
                if (IsDust(txout, wallet.chain().relayDustFee())) {
                    if (txout.nValue.IsExplicit() && txout.nValue.GetAmount() < 0) {
        ...
        current_fee = result.GetSelectedValue() - CalculateOutputValue(txNew);
        if (fee_needed != current_fee) {
            LogPrintf("WARNING: SFFO fee mismatch\n");
        }
    }
    ...
    if (fee_needed > current_fee) {
        LogPrintf("WARNING: Fee needed > fee paid (fee_needed=%lld, current_fee=%lld)\n", fee_needed, current_fee);
    }
