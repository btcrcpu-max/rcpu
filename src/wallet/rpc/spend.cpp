...
RPCHelpMan sendtoaddress()
{
    return RPCHelpMan{"sendtoaddress",
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    // !RCPU
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The RCPU address to send to."},
                    // !RCPU END
                    ...
                    // !RCPU
                                         "The recipient will receive less RCPU than you enter in the amount field."},
                    // !RCPU END
                ...
                RPCExamples{
                    ...
                    "\nSend 0.1 RCPU with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"donation\" \"sean's outpost\" false true 6 economical") +
                    "\nSend 0.1 RCPU with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB, subtract fee from amount, BIP125-replaceable, using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"drinks\" \"room77\" true true null \"unset\" null 1.1") +
                    "\nSend 0.2 RCPU with a confirmation target of 6 blocks in economical fee estimate mode using named arguments\n"
                    + HelpExampleCli("-named sendtoaddress", "address=\"" + EXAMPLE_ADDRESS[0] + "\" amount=0.2 conf_target=6 estimate_mode=\"economical\"") +
                    "\nSend 0.5 RCPU with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
                    // !RCPU END
...
// RCPU Silent Payments (T1): one-time output key derivation.
//
// Receiver publishes a Silent address = bech32("rs", spend_pub33 || scan_pub33).
// Sender uses one of its wallet input private keys a (public A = a*G):
//   shared = <a * B_scan>   (compressed point, 33B)
//   tweak  = SHA256("RCPU-Silent/v1" || shared33 || outpoint(txid32||voutLE4) || label(0x00)) mod n
//   P      = B_spend + tweak*G
// and pays an output to P2WPKH(HASH160(P)). The receiver scans a tx input
// (inputPub=A, outpoint) and recomputes the same P via ECDH (a*B_scan == b_scan*A);
// the matching output is spendable with d = b_spend + tweak (mod n).
namespace silentpayment {

//! Bech32 human-readable part used by RCPU silent addresses (e.g. "rs1...").
static constexpr const char* HRP = "rs";

namespace {

// secp256k1 curve order n, 32 bytes big-endian.
static const unsigned char SCALAR_N[32] = {
    0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFE,
    0xBA,0xAE,0xDC,0xE6,0xAF,0x48,0xA0,0x3B,0xBF,0xD2,0x5E,0x8C,0xD0,0x36,0x41,0x41
};

// Domain-separation prefix, byte-for-byte identical to the web wallet implementation.
static const unsigned char DOMAIN[14] = {'R','C','P','U','-','S','i','l','e','n','t','/','v','1'};
static const unsigned char LABEL[1] = {0x00};
...
bool Ge32(const unsigned char* a, const unsigned char* b)
{
    for (int i = 0; i < 32; ++i) {
...
// a = a - b (big-endian, requires a >= b)
void Sub32(unsigned char* a, const unsigned char* b)
{
    int borrow = 0;
    for (int i = 31; i >= 0; --i) {
        int diff = a[i] - b[i] - borrow;
        if (diff < 0) { diff += 256; borrow = 1; } else { borrow = 0; }
        a[i] = static_cast<unsigned char>(diff);
    }
}

// Reduce a 32-byte big-endian number modulo the secp256k1 group order n.
// Since in < 2^256 < 2n, a single subtraction suffices.
void ReduceScalar32(const unsigned char* in, unsigned char* out)
{
    std::memcpy(out, in, 32);
    if (Ge32(out, SCALAR_N)) {
        Sub32(out, SCALAR_N);
    }
}

// Convert 5-bit bech32 payload groups back to bytes (no padding required).
std::vector<unsigned char> ConvertBits(const std::vector<unsigned char>& in, unsigned frombits, unsigned tobits, bool pad)
{
    unsigned acc = 0, bits = 0;
    const unsigned maxv = (1u << tobits) - 1;
    const unsigned max_acc = (1u << (frombits + tobits - 1)) - 1;
    ...
    for (unsigned char value : in) {
        ...
        while (bits >= tobits) {
            bits -= tobits;
            ret.push_back(static_cast<unsigned char>((acc >> bits) & maxv));
        }
    }
    if (pad && bits > 0) {
        ret.push_back(static_cast<unsigned char>((acc << (tobits - bits)) & maxv));
    }
    return ret;
}
...
static std::vector<unsigned char> Bech32HrpExpand(const std::string& hrp)
{
    std::vector<unsigned char> ret;
    for (char c : hrp) {
...
static uint32_t Bech32Polymod(const std::vector<unsigned char>& values)
{
...
// Bech32 decoder that, unlike Bitcoin Core's bech32::Decode, accepts payloads
// longer than 90 chars, which is required for 66-byte RCPU silent addresses.
static bool Bech32Decode(std::string& hrp, std::vector<unsigned char>& data, const std::string& str)
{
    ...
    for (size_t i = sep + 1; i < str.size(); ++i) {
        const char* found = strchr(BECH32_CHARSET, str[i]);
        if (found == nullptr) {
            return false;
        }
        data.push_back(static_cast<unsigned char>(found - BECH32_CHARSET));
    }
    if (data.size() < 6) {
        return false;
    }
    std::vector<unsigned char> values = Bech32HrpExpand(hrp);
...
bool DecodeAddress(const std::string& addr, CPubKey& spend_pub, CPubKey& scan_pub)
{
    std::string hrp;
    std::vector<unsigned char> five;
    if (!Bech32Decode(hrp, five, addr) || hrp != HRP) {
        return false;
    }
    const std::vector<unsigned char> bytes = ConvertBits(five, 5, 8, false);
    if (bytes.size() != 66) {
        return false;
    }
    CPubKey sp, sc;
    sc.Set(&bytes[33], &bytes[66]);
    sp.Set(&bytes[0], &bytes[33]);
    if (!sp.IsFullyValid() || !sc.IsFullyValid()) {
        return false;
    }
    spend_pub = std::move(sp);
    scan_pub = std::move(sc);
    return true;
}

bool DeriveOutput(const CKey& sender_priv, const uint256& prev_txid, uint32_t vout,
                  const CPubKey& spend_pub, const CPubKey& scan_pub, CPubKey& out)
{
    if (!sender_priv.IsValid()) {
        return false;
    }
    if (!spend_pub.IsFullyValid() || !scan_pub.IsFullyValid()) {
        return false;
    }
    secp256k1_context* ctx = GetSilentContext();
    if (ctx == nullptr) {
        return false;
    }
    ...
    std::memcpy(scalar, sender_priv.begin(), sizeof(scalar));

    // shared = sender_priv * scan_pub  (scalar-multiply the peer point)
    secp256k1_pubkey shared_pub;
    if (secp256k1_ec_pubkey_parse(ctx, &shared_pub, scan_pub.data(), scan_pub.size()) != 1) {
        return false;
    }
    if (secp256k1_ec_pubkey_tweak_mul(ctx, &shared_pub, scalar) != 1) {
        return false;
    }
    unsigned char shared33[33];
    size_t shared_len = sizeof(shared33);
    if (secp256k1_ec_pubkey_serialize(ctx, shared33, &shared_len, &shared_pub, SECP256K1_EC_COMPRESSED) != 1) {
        return false;
    }

    // outpoint = txid(32B chain order) || vout(4B little-endian)
    unsigned char vle[4];
    vle[0] = static_cast<unsigned char>(vout & 0xff);
    vle[1] = static_cast<unsigned char>((vout >> 8) & 0xff);
    vle[2] = static_cast<unsigned char>((vout >> 16) & 0xff);
    vle[3] = static_cast<unsigned char>((vout >> 24) & 0xff);

    // tweak = SHA256(DOMAIN || shared33 || txid32 || voutLE4 || LABEL) mod n
    uint256 thash;
    {
        CSHA256 sha;
        sha.Write(DOMAIN, sizeof(DOMAIN));
        sha.Write(shared33, shared_len);
        // txid hashed in display (big-endian) order to match the web-wallet scanner outpointBytes(txidHex,vout)
        unsigned char txid_display[32];
        for (int _i = 0; _i < 32; ++_i) txid_display[_i] = prev_txid.begin()[31 - _i];
        sha.Write(txid_display, 32);
        sha.Write(vle, sizeof(vle));
        sha.Write(LABEL, sizeof(LABEL));
        sha.Finalize(thash.begin());
    }
    unsigned char tweak32[32];
    ReduceScalar32(thash.begin(), tweak32);

    // P = spend_pub + tweak*G
    secp256k1_pubkey out_pub;
    if (secp256k1_ec_pubkey_parse(ctx, &out_pub, spend_pub.data(), spend_pub.size()) != 1) {
        return false;
    }
    if (secp256k1_ec_pubkey_tweak_add(ctx, &out_pub, tweak32) != 1) {
        return false;
    }
    unsigned char out33[33];
    size_t out_len = sizeof(out33);
    if (secp256k1_ec_pubkey_serialize(ctx, out33, &out_len, &out_pub, SECP256K1_EC_COMPRESSED) != 1) {
        return false;
    }

    CPubKey pub;
    pub.Set(out33, out33 + out_len);
    if (!pub.IsFullyValid()) {
        return false;
    }
    out = std::move(pub);
...
// RCPU Silent Payments: send an amount to a one-time output derived from a silent
// address. The wallet pre-selects one of its spendable inputs to anchor the ECDH
// derivation, so the receiver can scan that input on-chain and recompute the output.
static UniValue SilentSend(const std::shared_ptr<CWallet>& pwallet, const std::string& silent_addr,
                           const CAmount& nAmount, mapValue_t& mapValue)
{
    EnsureWalletIsUnlocked(*pwallet);
    ...
    CPubKey spend_pub, scan_pub;
    if (!silentpayment::DecodeAddress(silent_addr, spend_pub, scan_pub)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid RCPU Silent address");
    }

    // Locate a spendable, key-owning coin to serve as the ECDH trigger input.
    CCoinControl coin_control;
    CKey trigger_key;
    COutPoint trigger_op;
    bool found = false;
    {
        CoinsResult coins = AvailableCoins(*pwallet);
        for (const COutput& coin : coins.All()) {
            ...
            if (std::holds_alternative<PKHash>(dest)) {
                ...
                keyid = ToKeyID(std::get<WitnessV0KeyHash>(dest));
            } else {
                continue;
            }
            ...
            CKey key;
            bool have = false;
            for (ScriptPubKeyMan* man : pwallet->GetScriptPubKeyMans(txout.scriptPubKey)) {
                ...
                if (auto* desc = dynamic_cast<DescriptorScriptPubKeyMan*>(man)) {
                    CKeyID kd;
                    if (desc->GetPrivKey(txout.scriptPubKey, kd, key)) { have = true; break; }
                } else {
                    std::unique_ptr<SigningProvider> spk = man->GetSolvingProvider(txout.scriptPubKey);
                    if (spk && spk->GetKey(keyid, key)) { have = true; break; }
                }
            }
            if (!have) continue;

            trigger_op = coin.outpoint;
            trigger_key = key;
            coin_control.Select(trigger_op);
            found = true;
            break;
        }
    }
    if (!found) {
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS,
                           "No spendable RCPU input with private keys is available to anchor the silent payment");
    }

    // Derive the one-time output public key paid to this transaction.
    CPubKey out_pub;
    if (!silentpayment::DeriveOutput(trigger_key, trigger_op.hash, trigger_op.n, spend_pub, scan_pub, out_pub)) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Failed to derive the silent payment output key");
    }

    CTxDestination dest = WitnessV0KeyHash(out_pub);
    std::vector<CRecipient> recipients;
    recipients.emplace_back(CRecipient{dest, nAmount, /*fSubtractFeeFromAmount=*/false});
...
RPCHelpMan silentsend()
{
    return RPCHelpMan{"silentsend",
        "Send an amount to an RCPU Silent (one-time) address." +
        HELP_REQUIRING_PASSPHRASE,
        {
            // !RCPU
            {"silentaddress", RPCArg::Type::STR, RPCArg::Optional::NO, "The RCPU Silent address (rs1...) to send to."},
            // !RCPU END
            {"amount", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The amount in " + CURRENCY_UNIT + " to send. eg 0.1"},
            {"comment", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "A comment used to store what the transaction is for.\n"
                                         "This is not part of the transaction, just kept in your wallet."},
        ...
        RPCExamples{
            // !RCPU
            "\nSend 0.1 RCPU to a silent address\n"
            + HelpExampleCli("silentsend", "\"rs1...\" 0.1") +
            // !RCPU END
            "\nSend 0.1 RCPU to a silent address with a comment using named arguments\n"
            + HelpExampleCli("-named silentsend", "silentaddress=\"rs1...\" amount=0.1 comment=\"hello\"")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
    ...
    const std::string silent_addr = request.params[0].get_str();
    const CAmount nAmount = AmountFromValue(request.params[1]);
    return SilentSend(pwallet, silent_addr, nAmount, mapValue);
},
    };
}
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::NO, "The addresses and amounts",
                        {
                            // !RCPU
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The RCPU address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                            // !RCPU END
                        },
                    },
                    ...
                    {"subtractfeefrom", RPCArg::Type::ARR, RPCArg::Optional::OMITTED, "The addresses.\n"
                                       ...
                                       // !RCPU
                                       "Those recipients will receive less RCPU than you enter in their corresponding amount field.\n"
                                       // !RCPU END
...
// !RCPU sendct: broadcast a confidential (blinded) transaction from explicit inputs.
RPCHelpMan sendct()
{
    return RPCHelpMan{"sendct",
        "\nCreate and broadcast a confidential (blinded) transaction that spends the given UTXO(s) to the "
        "given addresses. All paid amounts are blinded with Pedersen commitments (hidden on-chain); only the "
        "miner fee is left explicit.\n"
        + HELP_REQUIRING_PASSPHRASE,
        {
            {"inputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "The UTXOs to spend (must be owned or tracked by this wallet).",
                {
                    {"input", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "The UTXO to spend.",
                        {
                            {"txid", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The transaction id of the input UTXO."},
                            {"vout", RPCArg::Type::NUM, RPCArg::Optional::NO, "The output number (n) of the input UTXO."},
                        }},
                }},
            {"outputs", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::NO, "The addresses and amounts",
                {
                    // !RCPU
                    {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The RCPU address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                    // !RCPU END
        ...
        RPCResult{RPCResult::Type::STR_HEX, "txid", "The transaction id of the sent confidential transaction."},
        RPCExamples{
            "\nSend 100 RCPU from a specific UTXO as a confidential transaction:\n"
            + HelpExampleCli("sendct", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" \"{\\\"" + EXAMPLE_ADDRESS[0] + "\\\":100}\"")
            + HelpExampleRpc("sendct", "[{\"txid\":\"myid\",\"vout\":0}], {" + EXAMPLE_ADDRESS[0] + ": 100}")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
    ...
    // 1) Hand-select the exact inputs that must be spent (e.g. a specific coinbase UTXO).
    const UniValue inputs = request.params[0].get_array();
    CCoinControl coin_control;
    for (unsigned int i = 0; i < inputs.size(); ++i) {
        const UniValue& input = inputs[i].get_obj();
        const Txid txid = Txid::FromUint256(ParseHashO(input, "txid"));
        const int nOutput = input["vout"].getInt<int>();
        if (nOutput < 0) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, vout cannot be negative");
        }
        const COutPoint outpt(txid, nOutput);
        coin_control.Select(outpt);
    }
    if (!coin_control.HasSelected()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "sendct requires at least one input UTXO");
    }
    // Spend only the listed inputs; any change is returned to a wallet address.
    ...
    std::vector<CRecipient> recipients = CreateRecipients(
            ParseOutputs(sendTo),
            InterpretSubtractFeeFromOutputInstructions(UniValue(UniValue::VARR), sendTo.getKeys())
    );
    if (recipients.empty()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "sendct requires at least one output");
    }

    // 3) CreateTransaction() sets CT_VERSION and calls BlindTransaction() (see wallet/spend.cpp),
    //    then the wallet signs and broadcasts. Returns the new txid.
    return SendMoney(wallet, coin_control, recipients, /*map_value=*/{}, /*verbose=*/false);
},
    };
}
...
static std::vector<RPCArg> OutputsDoc()
{
    return
    {
        {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
            {
                // !RCPU
                {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the RCPU address,\n"
                // !RCPU END
        ...
        "\nSend a transaction.\n",
        {
            ...
            {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                Cat<std::vector<RPCArg>>(
                {
                    ...
                    // !RCPU
                    {"change_address", RPCArg::Type::STR, RPCArg::DefaultHint{"automatic"}, "The RCPU address to receive the change"},
                    // !RCPU END
    ...
    return RPCHelpMan{"sendall",
        ...
        "If your wallet contains many small inputs, either because it received tiny payments or as a result of accumulating change, consider using `send_max` to exclude inputs that are worth less than the fees needed to spend them.\n",
        {
            {"recipients", RPCArg::Type::ARR, RPCArg::Optional::NO, "The sendall destinations. Each address may only appear once.\n"
                ...
                {
                    // !RCPU
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "A RCPU address which receives an equal share of the unspecified amount."},
                    // !RCPU END
                    {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
                        {
                            // !RCPU
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (options.exists("inputs") && options.exists("send_max")) {
                ...
                for (const CTxIn& input : rawTx.vin) {
                    ...
                    total_input_value += (tx->tx->vout[input.prevout.n].nValue.IsExplicit() ? tx->tx->vout[input.prevout.n].nValue.GetAmount() : 0);
                }
            } else {
                ...
                for (const COutput& output : AvailableCoins(*pwallet, &coin_control, fee_rate, coins_params).All()) {
                    ...
                    if (send_max && fee_rate.GetFee(output.input_bytes) > (output.txout.nValue.IsExplicit() ? output.txout.nValue.GetAmount() : 0)) {
                        continue;
                    }
                    CTxIn input(output.outpoint.hash, output.outpoint.n, CScript(), rbf ? MAX_BIP125_RBF_SEQUENCE : CTxIn::SEQUENCE_FINAL);
                    rawTx.vin.push_back(input);
                    total_input_value += (output.txout.nValue.IsExplicit() ? output.txout.nValue.GetAmount() : 0);
                }
            }
            ...
            for (const CTxOut& out : rawTx.vout) {
                output_amounts_claimed += (out.nValue.IsExplicit() ? out.nValue.GetAmount() : 0);
            }
            ...
            for (CTxOut& out : rawTx.vout) {
                ...
                if (addresses_without_amount.count(addr) > 0) {
                    ...
                    if (!gave_remaining_to_first) {
                        out.nValue = (out.nValue.IsExplicit() ? out.nValue.GetAmount() : 0) +  remainder % addresses_without_amount.size();
