...
public:
    ...
    /** Real value of the output (unblinded for CT outputs that belong to us). */
    CAmount m_real_value{0};
    ...
    {
        m_real_value = value;
        // Recompute effective_value if fee is set
        if (fee) {
            effective_value = m_real_value - fee.value();
        }
    }
