...
public:
    ...
    //! For RCPU Silent Payments: fetch the private key this descriptor owns for `script`,
    //! if any. Returns true and fills `keyid`/`key` when a private key is available.
    bool GetPrivKey(const CScript& script, CKeyID& keyid, CKey& key) const;
