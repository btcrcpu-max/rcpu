                                 ...
                                 CAmount& old_fee, CAmount& new_fee, CMutableTransaction& mtx, bool require_mine, const std::vector<CTxOut>& outputs, std::optional<uint32_t> original_change_index)
{
    ...
    for (const CTxIn& txin : wtx.tx->vin) {
        ...
        input_value += (coin.out.nValue.IsExplicit() ? coin.out.nValue.GetAmount() : 0);
    ...
    for (unsigned int i = 0; i < wtx.tx->vin.size(); ++i) {
        ...
        if (new_coin_control.IsExternalSelected(txin.prevout)) {
            ...
            TransactionSignatureChecker tx_checker(wtx.tx.get(), i, coin.out.nValue.IsExplicit() ? coin.out.nValue.GetAmount() : 0, txdata, MissingDataBehavior::FAIL);
    ...
    for (const auto& old_output : wtx.tx->vout) {
        output_value += (old_output.nValue.IsExplicit() ? old_output.nValue.GetAmount() : 0);
    }
