...
bool DescriptorScriptPubKeyMan::GetPrivKey(const CScript& script, CKeyID& keyid, CKey& key) const
{
    LOCK(cs_desc_man);
    auto it = m_map_script_pub_keys.find(script);
    if (it == m_map_script_pub_keys.end()) {
    ...
    std::unique_ptr<FlatSigningProvider> prov = GetSigningProvider(it->second, /*include_private=*/true);
    if (!prov) {
        return false;
    }
    for (const auto& entry : prov->keys) {
