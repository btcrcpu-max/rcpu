...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    # !RCPU
    write_config(os.path.join(datadir, 'rcpu.conf'), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
    # !RCPU END
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        # !RCPU
        datadir = temp_dir / "RCPU"
        # !RCPU END
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            # !RCPU
            datadir = temp_dir / "Library/Application Support/RCPU"
            # !RCPU END
        else:
            # !RCPU
            datadir = temp_dir / ".rcpu"
...
def append_config(datadir, options):
    # !RCPU
    with open(os.path.join(datadir, "rcpu.conf"), 'a', encoding='utf8') as f:
    # !RCPU END
...
def get_auth_cookie(datadir, chain):
    ...
    password = None
    # !RCPU
    if os.path.isfile(os.path.join(datadir, "rcpu.conf")):
        with open(os.path.join(datadir, "rcpu.conf"), 'r', encoding='utf8') as f:
    # !RCPU END
            for line in f:
